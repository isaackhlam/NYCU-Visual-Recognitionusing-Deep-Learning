...
def get_digits_from_pred(pred, threshold=0.5):
    ...
    for i in range(len(pred["scores"])):
        if pred["scores"][i] > threshold:
            category_id = (
                pred["labels"][i].item() - 1
            )  # id start from 1, digit start from 0
            if 0 <= category_id <= 9:
                detected_digits.append(str(category_id))

    if detected_digits:
        sorted_digits = sorted(
            zip(pred["boxes"].cpu().numpy(), detected_digits), key=lambda x: x[0][0]
        )
...
def predict(args, logger):
    ...
    test_data = FasterRCNNTestDataset(
        f"{args.data_path}/{args.test_data_name}", transform
    )
    ...
    with torch.no_grad():
        for batch in tqdm(test_dataloader):
            ...
            for i, pred in enumerate(preds):
                ...
                labels.append(
                    (paths[i], get_digits_from_pred(pred)),
                )
