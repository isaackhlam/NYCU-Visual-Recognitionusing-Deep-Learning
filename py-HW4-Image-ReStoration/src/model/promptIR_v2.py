...
class PromptIR(nn.Module):
    def __init__(
        ...
        ff_dim=2048,
        prompt_vocab=("rain", "snow"),  # You can extend this
        dropout=0.1,
    ):
        super(PromptIR, self).__init__()

        # Prompt vocabulary and embedding
        self.prompt_vocab = prompt_vocab
        self.prompt_to_id = {p: i for i, p in enumerate(prompt_vocab)}
        self.prompt_embedding = nn.Embedding(len(prompt_vocab), embed_dim)
    ...
    def forward(self, x, prompts: list[str]):
        batch_size, _, H, W = x.shape
        input_size = (H, W)
        ...
        # Get prompt embedding for this degradation type
        # prompt_id = torch.tensor(
            # [self.prompt_to_id[prompt]] * batch_size, device=x.device
        # )
        # prompt_token = self.prompt_embedding(prompt_id).unsqueeze(1)  # (B, 1, D)
        prompt_ids = torch.tensor(
            [self.prompt_to_id[p] for p in prompts], device=x.device
        )
        # Get prompt embeddings for each sample
        prompt_token = self.prompt_embedding(prompt_ids).unsqueeze(1)  # (B, 1, D)

        # Concatenate prompt token with image features
        features = torch.cat([prompt_token, image_features], dim=1)
